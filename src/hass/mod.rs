...
impl Hass {
    ...
    #[tool(description = "Fetches the current image from the greenhouse camera")]
    async fn greenhouse_camera(&self) -> Result<CallToolResult, McpError> {
        let url = format!("{}/camera_proxy/camera.greenhouse_camera", self.uri.api());
        ...
        let image_content = Content::image(base64_image, "image/jpeg".to_string());
